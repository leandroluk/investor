...
export class ListDocumentsToApproveQuery extends createClass(
  Query,
  z.object({
...
export class ListDocumentsToApproveResult extends createClass(
  z.object({
    items: z.array(DocumentView.schema).meta({description: 'List of documents'}),
    total: z.number().meta({description: 'Total number of documents', example: 100}),
    page: z.number().meta({description: 'Current page', example: 1}),
    limit: z.number().meta({description: 'Limit per page', example: 20}),
  })
) {}
