...
import {Broker, BrokerMessage} from '#/core/port/broker';
import {Injectable} from '@nestjs/common';
import {EventEmitter2} from '@nestjs/event-emitter';
import {Kafka, logLevel, type Consumer, type Producer} from 'kafkajs';
import {BrokerKafkaConfig} from './broker-kafka.config';
import {BrokerKafkaError} from './broker-kafka.error';
...
export class BrokerKafkaAdapter implements Broker {
  ...
  async publish<TType extends object = any>(message: BrokerMessage<TType>): Promise<void> {
  ...
  async consume<TType extends object = any>(handler: (message: BrokerMessage<TType>) => void): Promise<void> {
