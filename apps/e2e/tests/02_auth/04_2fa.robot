...
Complete 2FA Authentication Flow
    ...
    ${auth_data} =    Authorize 2FA    ${test_email}    ${otp}
...
Login With Activated Account
    ...
    [Arguments]    ${email}
    Login With Credentials    ${email}    Test@123    202
...
Authorize 2FA
    ...
    ...    email=${email}
    ...    otp=${otp}
    ${response} =    PATCH On Session    api_session    ${2FA_PATH}    json=&{payload}    expected_status=200
