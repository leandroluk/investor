...
##### 01.31.⚡✅⚙️💨`[user]` send welcome email (internal)
- Disparado pela `UserSaga` ao receber `UserActivatedEvent`.
- Busca o email do usuário pelo `userId`. Se o usuário não existir, lança `UserNotFoundError`.
- Renderiza os templates `welcome.html` e `welcome.text` com a URL de onboarding.
- Envia o e-mail de boas-vindas com subject "Welcome to Investor!".
##### 01.32.⚡✅⚙️💨`[user]` send kyc pending notice (internal)
- Disparado pela `UserSaga` ao receber `UserActivatedEvent` (junto com 01.31).
- Busca o email do usuário pelo `userId`. Se o usuário não existir, lança `UserNotFoundError`.
- Renderiza os templates `kyc-none.html` e `kyc-none.text` com a URL de onboarding.
- Envia o e-mail informando que o processo de KYC deve ser iniciado, com subject "Start your KYC process".
##### 01.33.⚡✅⚙️💨`[user]` send kyc approved notice (internal)
- Disparado pela `UserSaga` ao receber `UserKycStatusChangedEvent` com `status === APPROVED`.
- Busca o email do usuário pelo `userId`. Se o usuário não existir, lança `UserNotFoundError`.
- Renderiza os templates `kyc-approved.html` e `kyc-approved.text` com a URL de onboarding.
- Envia o e-mail informando que a conta foi verificada, com subject "Your account has been verified!".
##### 01.34.⚡✅⚙️💨`[user]` complete onboard (internal)
- Disparado pela `UserSaga` ao receber `WalletLinkedEvent`.
- Busca o usuário pelo `userId`. Se não existir, lança `UserNotFoundError`.
- Se `isOnboardCompleted` já for `true`, a operação é idempotente (retorna sem alteração).
- Caso contrário, marca `isOnboardCompleted = true` e atualiza `updatedAt`.
##### 01.35.⚡✅⚙️💨`[admin]` consolidate kyc status (internal)
- Disparado pela `AdminSaga` ao receber `DocumentReviewedEvent` com `documentStatus === APPROVED`.
- Busca todos os documentos do usuário e consolida o status geral de aprovação.
- Regra de aprovação: Selfie aprovada **E** (RG frente+verso aprovados **OU** CNH frente+verso aprovados).
- Se todos os requisitos forem atendidos e o KYC ainda não estiver APPROVED, atualiza o status para `APPROVED` e registra `verifiedAt`.
- Publica `UserKycStatusChangedEvent` para dar continuidade ao fluxo da `UserSaga`.
##### 01.36.⚡✅⚙️💨`[audit]` register activity (internal)
- Disparado pela `AuditSaga` ao receber qualquer evento decorado com `@Auditable`.
- Extrai a `action` e o `metadata` do decorator do evento.
- Cria um registro em `tb_activity` usando o `correlationId` como `id` e `occurredAt` como `createdAt`.
- Em caso de falha na persistência, registra o erro via `LoggerPort` sem propagar a exceção (fail-safe).
##### 01.37.⚡⛔🔒📜`[withdrawal]` create withdrawal request
...
##### 01.38.⚡⛔🔒💨`[withdrawal]` process withdrawal payout
...
##### 01.39.⚡⛔🔒📜`[withdrawal]` confirm withdrawal (2FA check)
...
##### 01.40.🔍⛔🔒💨`[withdrawal]` list withdrawals
...
##### 01.41.🔄⛔🌎💨`[withdrawal]` dispatch coordinates 2fa verification, balance locking and blockchain execution
...
### 04. Signal (Alerts & Communication)
##### 04.01.⚡⛔🔒💨`[notice]` send notice (push/in-app)
...
##### 04.02.⚡⛔🔒💨`[notice]` mark notice as read
...
##### 04.03.🔍⛔🔒💨`[notice]` list notices (inbox)
...
### 05. System (Health & Configuration)
##### 05.01.🔍✅🌎💨`[system]` healthcheck
- Valida se o banco de dados, cache, mensageria e storage estão operacionais.
##### 05.02.🔍⛔🌎💨`[system]` get configuration
...
##### 05.03.⚡⛔🔒💨`[system]` send support ticket/feedback
